...
from store.logic import (
    ...
    create_update_product_categories,
    create_update_product_promotions,
...
class TestProductLogic:
    ...
    def test_get_product_details(self, create_product):
        ...
        assert list(product_details.categories.all()) == list(
            product_obj.categories.all()
        )
    ...
    def test_create_update_product_categories(self, create_product, create_category):
        ...
        create_update_product_categories(
            product_obj=create_product, categories=categories
        )
        updated_product = Product.objects.get(id=create_product.id)

        assert (
            list(updated_product.categories.values_list("id", flat=True)) == categories
        )
        assert Product.objects.count() == 1
    ...
    def test_create_update_product_promotions(self, create_product, create_promotion):
        ...
        create_update_product_promotions(
            product_obj=create_product, promotions=promotions
        )
        updated_product = Product.objects.get(id=create_product.id)

        assert (
            list(updated_product.promotions.values_list("id", flat=True)) == promotions
        )
        assert Product.objects.count() == 1
...
@pytest.mark.django_db
class TestCartLogic:
    def test_get_or_create_user_cart_if_user_has_carts(
    ...
    def test_get_cart_history_if_user_has_not_completed_cart(
...
class TestCartItemLogic:
    def test_get_cart_item_if_cart_item_exist(
    ...
    def test_update_cart_item(self, create_cart_item_fxt, create_product):
        (
            ...
            _,
            _,
        ) = create_cart_item_fxt
        product_obj = create_product
        update_cart_item(
            cart_item_obj=cart_item_obj, product_obj=product_obj, quantity=15
        )
    ...
    def test_delete_cart_item(self, create_cart_item_fxt, create_product):
        ...
        assert Product.objects.count() == 1
        assert CartItem.objects.count() == 0
        assert updated_on_stock.on_stock == int(
            initial_on_stock + cart_item_obj.quantity
        )

    def test_create_cart_item_fxt(
        self,
        create_not_completed_cart,
        create_product,
    ):
    ...
    def test_create_update_delete_cart_item_update_situation(
        ...
        assert product_obj.on_stock == initial_product_on_stock + (
            initial_cart_item_quantity - quantity
        )

    def test_create_delete_cart_item_delete_situation(
        self, create_cart_item_fxt, create_not_completed_cart, create_product
    ):
        cart_item_obj, _, cart_obj = create_cart_item_fxt
        product_obj = create_product
